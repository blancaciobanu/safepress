...
import {
  ...
  Plus, Search, Shield,
  Smartphone, Lock, Radio, Scale, Users,
...
import { useAMAs } from '../features/community/hooks/useAMAs';
...
/* Helper: returns array of category IDs on a post, handling both old
...
const DispatchCard = ({ post, onNavigate }) => {
  if (!post) return null;
  const catIds = getPostCatIds(post);
  ...
  return (
    <motion.div className="mb-8" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="py-4 border-b border-ink/12 cursor-pointer group" onClick={() => onNavigate(post.id)}>
        {catNames.length > 0 && (
          <div className="flex gap-1.5 mb-3">
            {catNames.map((n) => (
              <span key={n} className="font-mono text-[9px] tracking-widest uppercase px-1.5 py-0.5 border border-ink/15 text-smoke-dim">{n}</span>
            ))}
          </div>
        )}
...
const CategoryPills = ({ post }) => {
  const ids = getPostCatIds(post);
  ...
  return (
    <div className="flex flex-wrap gap-1 mt-2">
      {ids.map((catId) => {
        ...
        return cat ? (
          <span key={catId}
            className="font-mono text-[9px] tracking-widest uppercase px-1.5 py-0.5 border border-ink/15 text-smoke-dim">
            {cat.name}
          </span>
        ) : null;
      })}
...
const Community = () => {
  ...
  const { amas, loading: amasLoading } = useAMAs();
  ...
  const catCount = (catId) =>
    catId === 'all'
      ...
      : posts.filter((p) => p.type === currentTabType
          && (p.categories?.includes(catId) || p.category === catId)).length;
      ...
      <div>
        ...
        <motion.header initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex border-b border-ink/14 mt-8">
            {[
              ...
              { id: 'ama',         label: 'AMAs',        type: null        },
            ].map((tab, i) => {
              const active = activeTab === tab.id;
              const count  = tab.type ? posts.filter((p) => p.type === tab.type).length
                : amas.length;
              return (
                <button key={tab.id}
                  onClick={() => {
        ...
        {!isAMATab && (
          ...
          >
                ...
                return (
                  <span key={opt.id} className="flex items-center gap-2">
                    <button onClick={() => setSortMode(opt.id)}
                      className={`font-mono text-[10px] tracking-widest uppercase transition-colors ${active ? 'text-ink' : 'text-smoke-dim hover:text-ink-soft'}`}>
                      {opt.label}
                    </button>
            ...
            <button
              onClick={() => navigate(isQA ? '/community/new?type=question' : '/community/new')}
        ...
        <div className="lg:grid lg:grid-cols-[176px_1fr_256px] lg:gap-8 lg:items-start">
          ...
          {/* ── Category sidebar (desktop, hidden on AMAs tab) ───── */}
          {!isAMATab && (
          ...
          {/* ── Main column ──────────────────────────────────────── */}
          <div className={`min-w-0 ${isAMATab ? 'lg:col-span-2' : ''}`}>
              ...
              <div>
                {isSpecialist && !myAMA && (
                  <div className="border-t border-ink/15 pt-6 mb-8">
                    <p className="eyebrow sm text-oxblood mb-2">Launch your AMA</p>
                    <p className="text-sm text-smoke-dim mb-4 max-w-prose leading-relaxed">
                      Open a permanent thread where journalists can ask you anything.
                      You answer on your own schedule — no time pressure.
                    </p>
                    <button onClick={() => navigate('/community/new?type=ama')}
                      className="btn flex items-center gap-2 text-xs uppercase tracking-widest">
                ...
                {amasLoading ? (
                  ...
                  <div className="text-center py-16 border-t border-ink/10">
                    <p className="text-smoke text-sm mb-1">No AMAs yet</p>
                    <p className="text-smoke-dim text-xs">
                      Verified specialists can start a thread where journalists ask them anything.
                    </p>
                  </div>
                ) : (
                  <div>
                        ...
                        >
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {ama.specialistBio && (
                                <p className="text-sm text-smoke-dim italic leading-relaxed line-clamp-2 mb-2">
                                  {ama.specialistBio}
                                </p>
                              )}
                              <div className="flex items-center gap-3">
                                ...
                                {isRecent && qCount > 0 && (
                                  <span className="font-mono text-[9px] text-brass tracking-widest uppercase">active</span>
                                )}
                              </div>
                            </div>
                            <ArrowRight className="w-4 h-4 text-smoke-dim group-hover:text-ink transition-colors flex-shrink-0 mt-1" />
                          </div>
                        </div>
                      );
                    })}
                ...
                {loading ? (
                  ...
                  <>
                    {activeCategory === 'all' && !searchQuery.trim() && dispatchPost && (
                      <DispatchCard post={dispatchPost} onNavigate={(id) => navigate(`/community/${id}`)} />
                    )}
                    <div>
                      {feedPosts.map((post) => {
